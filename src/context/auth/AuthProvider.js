...
const AuthProvider = ({ children }) => {
  ...
  const register = async (formData) => {
    ...
    try {
      ...
      if(error.response && error.response.data && error.response.data.errors){
        ...
        toast("Registration failed. Please try again.", {position: "top-right", type: "error"})
      }
    }
  };
  ...
  const login = async (formData) => {
    try {
      ...
      if (res.data.token) {
        ...
        await getUser();
        toast("Login successful!", {position: "top-right", type: "success"})
      }
    } catch (error) {  
      ...
      if(error.response && error.response.data && error.response.data.errors){
        ...
        toast("Login failed. Please try again.", {position: "top-right", type: "error"})
      }
    }
  };
  ...
  const getUser = async () => {
    try {
      ...
      // If token is invalid/expired (401 or 403), clear it and logout
      ...
      if(error.response && error.response.data && error.response.data.errors){
        ...
        toast("Failed to get user data", {position: "top-right", type: "error"})
      }
    }
  };
