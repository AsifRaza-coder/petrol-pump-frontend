...
const Header = ({ title, subTitle, link, icon, dialog, btnIcon, setDialog, addBtnTitle, secondaryBtnTitle, secondaryBtnIcon, setSecondaryDialog, secondaryDialog }) => {
  ...
  return (
    ...
    >
      ...
      {/* Show button for dialog toggle (when setDialog is provided) */}
      {setDialog && !link && ( <Button sx={{mb:1, mt:1}} color="primary" onClick={()=>setDialog(!dialog)} variant="contained">
         {btnIcon && btnIcon} {addBtnTitle}
        </Button>)}
      {/* Show button for link navigation (when link is provided) */}
      {setDialog && link && ( <Button sx={{mb:1, mt:1}} color="primary" onClick={()=>setDialog(!dialog)} component={Link} to={link} variant="contained">
         {btnIcon && btnIcon} {addBtnTitle}
        </Button>)}
