...
const TotalSale = () => {
  ...
  useEffect(() => {
    if (filters.field === "date") {
      setFilter((prev) => ({ ...prev, operator: "inBetween" }));
    } else {
      if (filters.operator === "inBetween") {
        setFilter((prev) => ({ ...prev, operator: "$regex" }));
      }
    }
  ...
  return (
    <div className="m-0 mx-5 mb-4">
      ...
      <div className="bg-gradient-to-br from-white to-gray-50 rounded-xl shadow-xl border border-gray-100 mx-5 my-5 p-6 min-h-[500px] transition-all duration-300 hover:shadow-2xl">
