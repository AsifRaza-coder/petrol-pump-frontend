...
import { getAllActiveCustomers, getAllEmployees } from "../../redux/completeDataSlice/completeDataSlice";
import { clearCustomers } from "../../redux/customerSlice/customerSlice";

// SEARCH FORM FIELDS
const searchReportInput = (printReport, employees) => [
  {
    id: 1,
    label: "Employee",
    type: "select",
...
export default function EmployeeReport() {
  ...
  const printReport = async () => {
    const { employeeId, startDate, endDate } = state;
  ...
  return (
    <Box m="0px 20px 20px 20px">
      ...
      <Box sx={{ paddingLeft: 3, paddingRight: 3 }}>
        <GridForm
          inputs={searchReportInput(printReport, employees)}
          state={state}
          setState={setState}
      ...
      {data && (
        <div className="report-wrapper">
          <div className="report-card">
            <h1 className="main-title">Employee Statement</h1>

            <div style={{ display: "flex", justifyContent: "space-around" }}>
              <h4>Employee Name: {customer?.name}</h4>
              <h4>
                Current Advance:{" "}
